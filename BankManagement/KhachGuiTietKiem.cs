...
namespace BankManagement
{
    public partial class KhachGuiTietKiem : Form
    {
        ...
        private void KhachGuiTietKiem_Load(object sender, EventArgs e)
        {
            ...
            // Sử dụng tham số thay vì ghép chuỗi trực tiếp
            int sotaikhoan = db.GetSoTaiKhoanByTaiKhoan(TaiKhoanDung);
            string query = "SELECT * FROM TietKiem WHERE SoTaiKhoan = " + sotaikhoan;
        ...
        private void btnXacNhanGui_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (soTienGui > db.GetSoDuByTSoTaiKhoan(soTaiKhoan))
                {
...
new SqlParameter("@MaTietKiem", maTietKiem),
    ...
    new SqlParameter("@ThoiGian", thoiGian)
                ...
                int sotaikhoan = db.GetSoTaiKhoanByTaiKhoan(TaiKhoanDung);
                string query = "SELECT * FROM TietKiem WHERE SoTaiKhoan = " + sotaikhoan;
                SqlParameter[] parametersdoc = new SqlParameter[]
                {
                new SqlParameter("@SoTaiKhoan", TaiKhoanDung)
                };

                // Lấy dữ liệu từ cơ sở dữ liệu
                System.Data.DataTable dt = db.DocBang(query, parametersdoc);
        ...
        private void btnXacNhanRut_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                // Lấy số tiền còn lại trong tài khoản tiết kiệm
                string query = "SELECT SoTien FROM TietKiem WHERE MaTietKiem = @MaTietKiem";
                SqlParameter[] parameters = new SqlParameter[]
                {
            new SqlParameter("@MaTietKiem", txtMaKhoanVayTra.Text)
                };

                // Lấy kết quả trả về từ cơ sở dữ liệu
                object result = db.ThucThiGiaTriDon(query, parameters);

                if (result == null)
                {
                    MessageBox.Show("Mã tiết kiệm không tồn tại.");
                    return;
                }

                int SoTien = Convert.ToInt32(result); // Convert result to int
            ...
            new SqlParameter("@SoTienSauLai", IOManager.TinhSoTienSauLai(soTienRut,decimal.Parse(txtLaiSuat.Text),int.Parse(txtKiHan.Text)))
                ...
                db.CapNhatDuLieu(sqlUpdateTietKiem, parametersa);
            ...
            new SqlParameter("@SoTaiKhoan", soTaiKhoan)
                ...
                int sotaikhoan = db.GetSoTaiKhoanByTaiKhoan(TaiKhoanDung);
                string queryrut = "SELECT * FROM TietKiem WHERE SoTaiKhoan = " + sotaikhoan;
                SqlParameter[] parametersdoc = new SqlParameter[]
                {
                new SqlParameter("@SoTaiKhoan", TaiKhoanDung)
                };
                ...
                // Lấy dữ liệu từ cơ sở dữ liệu
                System.Data.DataTable dt = db.DocBang(queryrut, parametersdoc);
        ...
        private void Reset()
        {
            txtMaKhoanVayTra.Text = "";
            txtTaiKhoanRut.Text = "";
            txtHocTenRut.Text = "";
            txtHoatDongRutTien.Text = "";
            txtKiHan.Text = "";
            txtLaiSuat.Text = "";
            txtSoTienRut.Text = "";
            btnXacNhanRut.Enabled = true;
