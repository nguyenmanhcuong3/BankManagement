    ...
    public partial class BaoCao : Form
    {
        ...
        public BaoCao()
        {
            ...
            try
            {
                ...
                int soLuongTienTietKiem = GetSum("SELECT SUM(SoTien) FROM TietKiem", "Số lượng tiền gửi bằng 0\n");
                int soLuongTienVay = GetSum("SELECT SUM(SoTien) FROM KhoanVay", "Số lượng tiền vay = 0");

                // Cập nhật giá trị cho các Label
                lbslkh.Text = soLuongKhachHang.ToString();
                lbslgd.Text = soLuongGiaoDich.ToString();
                lbsltg.Text = soLuongTienTietKiem.ToString();
                lbsltv.Text = soLuongTienVay.ToString();

                // Vẽ hai biểu đồ
                DrawNguoiChart(soLuongKhachHang);
                DrawTienChart(soLuongTienTietKiem, soLuongTienVay);
            }
